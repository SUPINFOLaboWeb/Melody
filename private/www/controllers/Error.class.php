...
class Error extends \Core\Controller
{
	public function anyTestAction($req, $res, $code, $args)
	{
		...
		{
			list($uri, $msg) = unserialize(\core\Tools::base64url_decode($req->getArg(0)));
		}
		else
		{
			$msg = $args[0];
		...
		echo 'URL : /'.(isset($uri) ? $uri : \core\FrontController::$uri)."\n";
		echo 'Error : '.$code.' '.$this->MessageFor($code)."\n";
		echo 'Message : '.$msg."\n";

		return  $res->setView('buffer')->setHeader('Content-Type', 'text/plain');
	}
